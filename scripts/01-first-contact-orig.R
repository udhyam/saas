...
dups_phone_num <- data_master_vyapaaris |>
  filter(vyapaari_data_status == "existing") |>
...
# Vyapaari ID
# -----------------------------------------------------------------------------
vyapaari_ids <- read_sheet("https://docs.google.com/spreadsheets/d/1bcSvo_9ONt0gt8NNNNCT6W0pFBqtjzYBHjcO0YcWrEE/", 
                           sheet = "vyapaari-id-master")

data_master_vyapaaris <- data_master_vyapaaris |>
  mutate(vyapaari_name = str_trim(vyapaari_name)) |>
...
vyapaari_ids_new <- data_master_vyapaaris |>
  ...
  group_by(phone_number, vyapaari_name, vyapaari_id) |>
  summarise(1) |>
  arrange(vyapaari_id) |>
  select(-`1`)

write_sheet(vyapaari_ids_new, 
            ss = "https://docs.google.com/spreadsheets/d/1bcSvo_9ONt0gt8NNNNCT6W0pFBqtjzYBHjcO0YcWrEE/",
            sheet = "vyapaari-id-master")
